...
> Google × Kaggle 5-Day AI Agents Intensive — Day 2
> November 2025 | 55 pages
...
November 20252Acknowledgements
...
November 20257Introduction: Models, Tools and Agents
...
perceive and act on the world.Unifying Agents, Tools, and 
the World

November 20258With the advent of Agentic AI, tools become even more important to AI systems. An AI Agent 
...
November 20259requests, by accessing structured and unstructured data sources; or, tools can perform an 
...
November 202511Snippet 1: Definition for set_light_values tool
Built-in tools
...
Execution13, URL Context14, and Computer Use15.Python
...
November 202512This example below shows how to invoke the Gemini built-in url_context  tool. The tool 
definition itself is invisible to the developer; it's provided to the model separately.
 
 
Snippet 2: Calling url_context toolPython
...
November 202513Agent Tools
An agent can also be invoked as a tool. This prevents a full handoff of the user conversation, 
allowing the primary agent to maintain control over the interaction and process the subagent's input and output as needed. In ADK, this is accomplished by using the AgentTool16 
...
Snippet 3: AgentTool definition Python
...
November 202514Taxonomy of Agent Tools
...
Table 1: Tool categories & design considerationsTool Use Case Key Design Tips
Structured Data 
RetrievalQuerying databases, spreadsheets, or 
other structured data sources (e.g., MCP 
Toolbox, NL2SQL)Define clear schemas, optimize for efficient 
querying, handle data types gracefully.
Unstructured Data 
RetrievalSearching documents, web pages, or 
knowledge bases (e.g., RAG sample)Implement robust search algorithms, 
consider context window limitations, and 
provide clear retrieval instructions.
Connecting to 
Built-in TemplatesGenerating content from 
predefined templatesEnsure template parameters are welldefined, provide clear guidance on 
template selection.
Google Connectors Interacting with Google Workspace apps 
(e.g., Gmail, Drive, Calendar)Leverage Google APIs, ensure proper 
authentication and authorization, handle 
API rate limits.
Third-Party 
ConnectorsIntegrating with external services 
and applicationsDocument external API secifications, 
manage API keys securely, implement error 
handling for external calls.

November 202515Best Practices
...
November 202516• Add targeted examples: Examples can help address ambiguities, show how to handle 
...
Snippet 4: Good tool documentationPython
...
November 202517Snippet 5: Bad tool documentation
...
a file; document this so the agent knows how to access the data. Python
def fetchpd(pid):
  """
...
November 202518Publish tasks, not API calls
...
November 202519Design for concise output
...
November 202520also be provided to the calling LLM, so it provides another avenue for giving instructions. 
...
November 202521of external tools and data. By standardizing this communication layer, MCP aims to decouple 
...
may be a standalone application, or a sub-component of a larger system such as a multiagent system. Responsibilities include managing the user experience, orchestrating the 
...
November 202522Figure 2:  MCP Host, Client and Server in an Agentic Application
...
November 202523Base Protocol:  MCP uses JSON-RPC 2.0 as its base message format. This gives it a 
...
November 202525Of these capabilities defined by the MCP specification, only Tools are broadly supported. As 
the table below shows, while Tools are supported by nearly all tracked client applications, 
Resources and Prompts are only supported by approximately a third, and support for clientside capabilities is significantly lower than that. So it remains to be seen whether these 
...
and parameter schemas, for agents to discover.CapabilityClient Support Status
% Supported
...
November 202526Tool Definition
...
November 202527• openWorldHint : May interact with an "open world" of external entities (default: true).
...
November 202529MCP also allows Tools to return specified Resources, which gives developers more options 
...
server/tools#error-handling , retrieved 2025-09-16Python
{
  "jsonrpc" : "2.0",
  "id": 3,
  "error" : {
...
November 202531Other Capabilities
...
November 202532While they do have the potential to add value to an AI system, in a distributed enterprise 
environment the use of Prompts introduces some evident security concerns. Allowing a thirdparty service to inject arbitrary instructions into the execution path of the application is risky, 
...
November 202534used in a production MCP system. For one thing, there are no guardrails in the specification 
...
November 202535MCP Registry30, which provides both a central source of truth for public MCP servers, and 
...
November 202536Foundations for Governance and Control
...
November 202537Performance and Scalability Bottlenecks
...
November 202538This would transform tool discovery from a static, brute-force loading process into a 
...
November 202539MCP was designed for open, decentralized innovation, which spurred its rapid growth, and 
...
November 202540As a standard agent protocol , MCP is being used for a broad range of applications, 
...
November 202541retrieved via a tools/list  request. MCP Servers are also not required to notify clients when 
...
November 202542• Host MCP servers in a controlled environment:  Dynamic capability injection is a risk 
...
November 202543• Description: "Saves any important data from the user to a private, secure repository. Use 
...
MCP Client/Gateway should check for name collisions with existing, trusted tools. An LLMbased filter could be appropriate here (rather than an exact or partial name match) to 
...
November 202544• Restrict Access to Unauthorized MCP Servers:  In the example above the coding 
...
November 202545• Separate System Prompts:  Clearly separate user inputs from system instructions to 
prevent a user from tampering with core model behavior. Taking this a step further, one 
could build an agent with two separate planners, a trusted planner with access to firstparty or authenticated MCP tools, and an untrusted planner with access to third-party 
...
November 202550The MCP server has been granted extensive privileges to the code repository to perform 
...
edu/2023/04/19/gpt-4-passes-the-bar-exam-what-that-means-for-artificial-intelligence-tools-in-thelegal-industry/  [accessed 3 November 2025]
...
finds", MITSloan Management Review  (1 September 2025), https://sloanreview.mit.edu/article/agenticai-at-scale-redefining-management-for-a-superhuman-workforce/  [accessed 3 November 2025]
...
November 20255313. "Code Execution", Gemini API Docs , Google (2025),  
...
(A2A)", Google for Developers , Google (9 April 2025), https://developers.googleblog.com/en/a2a-a-newera-of-agent-interoperability/ . [accessed 3 November 2025]
18. "Artifacts", Agent Development Kit , Google (2025), https://google.github.io/adk-docs/artifacts/#artifactservice-baseartifactservice  [accessed 3 November 2025]
...
November 20255425. "Server Features: Prompts", Model Context Protocol Specification , Anthropic (2025),   
...
Registry", mcp blog , Anthropic (8 September 2025), https://blog.modelcontextprotocol.io/posts/202509-08-mcp-registry-preview/  [accessed 3 November 2025]
31. Gan, Tiantian, Qiyao Sun, "RAG-MCP: Mitigating Prompt Bloat in LLM Tool Selection via RetrievalAugmented Generation",  arXiv preprint arXiv:2505.03275  (2025) [accessed 3 November 2025]
...
Research (2025). https://research.google/pubs/an-introduction-to-googles-approach-for-secure-aiagents/  [accessed 3 November 2025]
...
November 20255536. Milanta, Marco, and Luca Beurer-Kellner, "GitHub MCP Exploited: Accessing private repositories via 
MCP", InvariantLabs (26 May 2025). https://invariantlabs.ai/blog/mcp-github-vulnerability  [accessed 3 
November 2025]
37. "Model Armor overview", Security Command Center , Google (2025) https://cloud.google.com/securitycommand-center/docs/model-armor-overview  [accessed 3 November 2025]
...
modelcontextprotocol.io/specification/2025-03-26/basic/authorization#2-2-example%3Aauthorization-code-grant  [accessed 3 November 2025]
